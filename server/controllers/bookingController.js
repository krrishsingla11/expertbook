...
exports.createBooking = async (req, res) => {
  try {
    ...
    if (io) {
      io.emit('slotBooked', { expertId, date, timeSlot, bookingId: booking._id });
    }
...
exports.updateBookingStatus = async (req, res) => {
  try {
    ...
    if (status === 'Cancelled') {
      ...
      const io = req.app.get('io');
      if (io) {
        io.emit('slotFreed', { expertId: booking.expertId, date: booking.date, timeSlot: booking.timeSlot });
      }
    }
